...
public class IdleItemController {
    ...
    public Result<?> publish(@RequestBody IdleItem item,
        ...
        if (redisService != null) {
            redisService.deleteByPattern("idle:list:*");
        }
    ...
    public Result<Page<Map<String, Object>>> list(
        ...
        String cacheKey = "idle:list:" + page + ":" + size + ":" + 
                         (category != null ? category : "") + ":" + 
                         (keyword != null ? keyword : "") + ":" + 
                         (status != null ? status : "") + ":" + 
                         (role != null ? role : "");
        
        if (redisService != null && redisService.hasKey(cacheKey)) {
            Page<Map<String, Object>> cached = (Page<Map<String, Object>>) redisService.get(cacheKey);
            System.out.println("[IdleCache] ✅ 命中缓存");
        ...
        if (redisService != null) {
            redisService.set(cacheKey, resultPage, 15, TimeUnit.MINUTES);
            System.out.println("[IdleCache] ❌ 未命中缓存，已写入");
        }
