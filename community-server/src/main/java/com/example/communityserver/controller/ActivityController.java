...
public class ActivityController {
    ...
    private static final long ACTIVITY_CACHE_EXPIRE_MINUTES = 10;
    ...
    public Result<Page<Map<String, Object>>> list(
        ...
        String cacheKey = ACTIVITY_LIST_CACHE_PREFIX + page + ":" + size + ":"
                + (status == null ? "" : status) + ":" + (keyword == null ? "" : keyword)
                + ":" + (userId == null ? "0" : userId);

        if (redisService != null) {
            Object cached = redisService.get(cacheKey);
            if (cached != null && cached instanceof Page) {
                System.out.println("[ActivityCache] ✅ 命中缓存, key=" + cacheKey);
                return Result.success((Page<Map<String, Object>>) cached);
            }
            System.out.println("[ActivityCache] ❌ 未命中缓存, key=" + cacheKey);
        }
        ...
        if (redisService != null) {
            redisService.set(cacheKey, resultPage, ACTIVITY_CACHE_EXPIRE_MINUTES, TimeUnit.MINUTES);
        }
    ...
    private void clearActivityCache() {
        if (redisService != null) {
            redisService.deleteByPattern(ACTIVITY_LIST_CACHE_PREFIX + "*");
        }
    }
}
