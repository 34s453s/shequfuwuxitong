...
public class RepairOrderController {
    ...
    public Result<?> submit(@RequestBody RepairOrder order,
        ...
        if (redisService != null) {
            redisService.deleteByPattern("repair:list:*");
        }
    ...
    public Result<Page<RepairOrder>> list(
        ...
        String cacheKey = "repair:list:" + page + ":" + size + ":" + 
                         (status != null ? status : "") + ":" + role;
        
        if (redisService != null && redisService.hasKey(cacheKey)) {
            Page<RepairOrder> cached = (Page<RepairOrder>) redisService.get(cacheKey);
            System.out.println("[RepairCache] ✅ 命中缓存");
        ...
        if (redisService != null) {
            ...
            System.out.println("[RepairCache] ❌ 未命中缓存，已写入");
        }
